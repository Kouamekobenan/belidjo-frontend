...
const Navbar = () => {
  ...
  const getRoleConfig = () => {
    ...
    if (isVendor)
      return {
        ...
        bg: "bg-orange-50",
        dot: "bg-orange-500",
        icon: <img src="/images/shop.png" alt="Shop" className="w-[18px] h-[18px] object-contain" />,
      };
    if (isClient)
      return {
        label: "Mode Client",
  ...
  const AuthButtons = ({ mobile = false }) => {
    if (isAuthenticated && roleConfig) {
      return (
        <Link
          ...
          className={`group inline-flex items-center gap-2 px-5 py-2.5 text-sm font-bold text-white bg-green-600 hover:bg-green-700 rounded-xl shadow-md transition-all ${mobile ? "w-full justify-center py-4" : ""}`}
        >
          {roleConfig.icon}
    ...
    return (
      <Link
        href="/users/ui/login"
        className={`group inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-white bg-green-600 hover:bg-green-700 rounded-xl shadow-lg transition-all ${mobile ? "w-full justify-center py-4" : ""}`}
      >
        <LogIn
          size={18}
    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-6 flex items-center gap-6">
          <Link href="/vendor" className="flex items-center gap-3 group shrink-0">
            ...
            <span
              className={`text-2xl font-black tracking-tighter ${isScrolled ? "text-gray-900" : "text-green-600"}`}
            >
              {cityName}
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-lg border-t border-gray-100 pb-safe">
        <div className="grid grid-cols-3 h-16">
          <Link
            href="/vendor"
            className="flex flex-col items-center justify-center text-gray-400 hover:text-green-600"
          >
            <Home size={22} />
          ...
          >
            <div className="bg-gray-900 text-white p-2 rounded-xl -mt-8 shadow-lg border-4 border-white flex items-center justify-center w-10 h-10">
